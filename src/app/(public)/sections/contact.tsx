...
export const HomeSectionContact = () => {
	...
	return (
		<div
			className={clsx("text-white justify-center items-center flex w-full shadow  h-full overflow-hidden", {
		...
		>
			<div className="flex flex-col">
				<div className="flex flex-col gap-2 px-20 py-14 text-center">
					<p
						className={clsx({
				...
				<div className="flex gap-6 h-full justify-start text-justify items-start max-w-max overflow-hidden">
					<div className="flex gap-4 px-20 py-14 text-center flex-wrap justify-center items-center ">
						{redes.map((rede) => (
							<a
								key={rede.title}
								href={rede.link}
								...
								className={clsx(
									...
									{
										"text-white font-bold bg-gradient-to-r from-violet-600 to-pink-400": themeSelection === "dark",
										"text-zinc-800 font-bold bg-gradient-to-r from-blue-400 to-cyan-600": themeSelection === "light",
									},
								)}
							>
								<div className="flex flex-col gap-2 items-center">
									{rede.icon}
									<h1 className={clsx("text-2xl sm:text-2xl l font-bold text-white")}>{rede.title}</h1>

									<p className={clsx("text-xs text-white")}>{rede.subtitle}</p>
								</div>
							</a>
						))}
					</div>
